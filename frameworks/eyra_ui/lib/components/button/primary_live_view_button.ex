defmodule EyraUI.Button.PrimaryLiveViewButton do
  ...
  def render(assigns) do
    ~H"""
    <button phx-click={{ @event }} class="pt-15px pb-15px active:pt-4 active:pb-14px active:shadow-top4px leading-none font-button text-button focus:outline-none rounded {{@bg_color}} {{@text_color}} {{@width}}">
      {{ @label }}
    </button>
    """
  end
