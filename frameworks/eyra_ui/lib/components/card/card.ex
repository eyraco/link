defmodule EyraUI.Card.Card do
  ...
  def handle_event("handle_click", _params, socket) do
    client_event_data =
      if socket.assigns.click_event_data,
        do: socket.assigns.click_event_data,
        else: socket.assigns.id

    send(self(), {:handle_click, client_event_data})
    {:noreply, socket}
  end

  def render(assigns) do
    ~H"""
    <div class="rounded-lg cursor-pointer {{@bg_color}} {{@size}}" :on-click="handle_click">
      <slot name="image" />
